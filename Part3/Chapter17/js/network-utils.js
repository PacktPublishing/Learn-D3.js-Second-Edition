/**
 * Network layout utilities for D3.js
 * Provides functions to convert between matrix and link representations of graphs,
 * as well as layout generators for adjacency matrices, arc diagrams, circle diagrams, and square grids.
...
export function linksToMatrix (nodes, links, objectIds = false, copyOnZero = false, source = "source", target = "target", value  = "value") {
    ...
    const pool = copyOnZero ? links.slice() : links; // thread-safe copy

    if (copyOnZero) {
        pool.forEach(link => {
            if (!pool.some(l => l[source] === link[target] && l[target] === link[source])) {
                pool.push({ [source]: link[target], [target]: link[source], [value]: link[value] });
            }
        });
    }

    const matrix = Array.from({ length: nodeCount }, (_, s) =>
        Array.from({ length: nodeCount }, (_, t) => {
            const match = !objectIds
                ? pool.find(k => k[source] === s && k[target] === t)
                : pool.find(k => k[source] === nodes[s] && k[target] === nodes[t]);
...
export function adjacencyMatrix() {
    ...
    function layout(nodes, sourceMatrix) {
        const cellW = w / nodes.length, cellH = h / nodes.length;

        return sourceMatrix.flatMap((row, rowIdx) =>
    ...
    layout.size = function(array) {
        return arguments.length ? (w = +array[0], h = +array[1], layout) : layout;
    }
