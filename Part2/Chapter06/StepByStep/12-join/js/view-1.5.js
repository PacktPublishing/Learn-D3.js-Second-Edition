 ...
 * Calls createMoons() for the enter selection and updateMoon() for the merged update selection.
 * @param plane The orbital plane (vertical center of the chart)
 */
function drawMoons(plane) {
    plane.selectAll("g.moon")
         .data(app.current.moons)
             .join(enter => createMoons(enter)) // handle the enter selection (when an object is added)
             .attr("transform", d => `translate(${[d.cx,0]})`) // handle both enter and update selections (merged)
             .each(function() {
...
function updateObjects(moon) {
    ...
    moon.select("circle")
        .attr("r", d => !d.image ? app.scale(d.diameterKm) / 2 : 0)
