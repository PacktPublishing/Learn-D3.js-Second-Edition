...
const source = 'data/un_population_estimates.csv';
const target = 'data/un_population_2025.csv';
...
fs.readFile(source, 'utf8', (err, data) => {
    ...
    const rows = d3.dsvFormat(";").parse(data, row => {
        const newRow = {};
        if (row.Type === 'Country/Area' && row.Year === '2025') { // keep only country/area and 2025 entries
            for (const key in row) {
                if (key !== 'Type' && key !== 'Year') { // remove 'Type' and 'Year' columns
